			...
			document.getElementById("delSelected").onclick = function(){
				if(confirm("您确定要删除选中条目吗？")){
					...
					else
					{
						...
						//上面会拼接出一个名为idAr的数组
						window.location.href = "${ctx}/tec/processStation/delmore?idAr="+idAr;
...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/tec/processStation">工序与工站关系列表</a></li>
	...
	<form:form id="searchForm" method="post" action="${ctx}/tec/processStation" modelAttribute="processStation" class="breadcrumb form-search">
		...
		<div class="controls">
			...
			<input id="PutExcel" type="submit" value="导出EXCEL" class="btn btn-primary" onclick="tableToExcel('contentTable','processStationList')"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="ps">
				<tr>
					<td><input type="checkbox" name="ids" id="id" value="${ps.id}"/></td>
					<td>${ps.process.proName}</td>
					<td>${ps.workStationInfos.stationName}</td>
					<td>${ps.workStationInfos.line.lineName}</td>
					<td>
						<a href="${ctx}/tec/processStation/form?id=${ps.id}">修改</a>
						<a href="${ctx}/tec/processStation/delete?id=${ps.id}" onclick="return confirmx('确认要删除该工序与工站关系吗？',this.href)">删除</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
